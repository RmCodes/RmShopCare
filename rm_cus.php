<?php
        ...
        {
            $SQL = "SELECT * FROM  cust_details WHERE cus_name LIKE '%".$_POST['searchtxt']."%' OR cus_address LIKE '%".$_POST['searchtxt']."%'";
        }
        $tbl_name="cust_details";
        $adjacents = 3;
        $query = "SELECT COUNT(*) as NUM FROM $tbl_name";
        if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
        {
            $query = "SELECT COUNT(*) as num FROM  cust_details WHERE cus_name LIKE '%".$_POST['searchtxt']."%' OR cus_address LIKE '%".$_POST['searchtxt']."%'";
        }
        ...
        if($page)
            ...
            $start = 0;
        $sql = "SELECT * FROM cust_details LIMIT $start, $limit ";
        if(isset($_POST['Search']) AND trim($_POST['searchtxt'])!="")
        {
            $sql= "SELECT * FROM  cust_details WHERE cus_name LIKE '%".$_POST['searchtxt']."%' OR cus_address LIKE '%".$_POST['searchtxt']."%' LIMIT $start, $limit";
        }
...
<input class="btn btn-primary dropdown-toggle" name="dsubmit" type="button" value="Delete Selected" style="margin-left:5px;" onClick="return confirmDeleteSubmit()"/>
                ...
                <div class="row">
                       ...
                       <!-- <form role="form" name="deletefiles" action="delet.php" method="post" >
                            <input name="table" type="hidden" value="cust_details">
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td align="center"><?php echo $row['cus_address']; ?></td>
                                                                     <td align="center"><?php echo $row['contact']; ?></td>
                                                                      <td align="center"><?php echo $row['contact2']; ?></td>
                                                                     
                                                                   
 <td align="center"> <a href="edit.php?cat_id=<?php echo $row['cus_id'];?>"><span class=" glyphicon glyphicon-pencil"></span></a></td>
   <td align="center"><a onClick="return confirmSubmit()" href="delete.php?cat_id=<?php echo $row['cus_id']; ?>&table=category&return=category.php"><span class="glyphicon glyphicon-trash"></span></a></td>
                                                                    <td align="center" ><input  type="checkbox" value="<?php echo $row['cus_id']; ?>" name="checklist[]" /></td>
                                                                </tr>
                                                                <?php
                                                            }
                                                            ?>
